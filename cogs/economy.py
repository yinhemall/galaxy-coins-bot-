    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if amt <= 0 or data.get(uid, {"balance": 0})["balance"] < amt:
                await interaction.response.send_message("❌ 餘額不足或金額無效！", ephemeral=True)
                return

            user_dice, bot_dice = random.randint(1, 6), random.randint(1, 6)
            if user_dice > bot_dice:
                data[uid]["balance"] += amt
                msg = f"🎉 **你贏了！** 點數: {user_dice} vs {bot_dice}，獲得 {amt} 銀河幣！"
            else:
                data[uid]["balance"] -= amt
                msg = f"💀 **你輸了！** 點數: {user_dice} vs {bot_dice}，失去 {amt} 銀河幣。"
